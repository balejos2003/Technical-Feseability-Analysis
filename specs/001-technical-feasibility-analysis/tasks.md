...
- [ ] T009 Implement SQLite schema creation and connection handling in `src/feasibility/storage.py`, including request, assessment, evidence, finding, and finding-evidence records with analyzer versioning.
- [ ] T010 Implement principal-scoped persistence queries in `src/feasibility/history.py`, ensuring every list, search, and show operation filters by `principal_id` and completed assessments are immutable history snapshots.
...
- [ ] T013 [US1] Implement the interactive main menu and session loop in `src/feasibility/interactive.py`, with actions for analyzing a change, consulting history, and exiting.
- [ ] T014 [US1] Implement guided prompts for repository path, requested change, optional context, and report-save preference in `src/feasibility/interactive.py`.
- [ ] T015 [US1] Implement input validation and retry behavior for blank changes, unavailable paths, invalid menu choices, and interrupted sessions in `src/feasibility/interactive.py`.
...
- [ ] T018 [P] [US2] Implement bounded source-context preparation for the AI in `src/feasibility/context.py`, preserving file paths, line ranges, excerpts, hashes, exclusions, and context-size limits.
